...
describe('SELECT.TournamentN', function() {
  ...
  describe('Tournament2', function() {
    ...
    it('should return winner of tournament', function() {
      const pop = { genes: _.range(10).map(i => wrapGene(i, i)) }
      const r = new TestRandom(4, 8)
      expect(Tournament2(pop, r)).to.equal(pop.genes[8])
    })
    it('should return first in tie', function() {
      const pop = { genes: _.range(10).map(i => wrapGene(i, 0)) }
      const r = new TestRandom(4, 8)
      expect(Tournament2(pop, r)).to.equal(pop.genes[4])
    })
  ...
  describe('Tournament3', function() {
    ...
    it('should return winner of tournament', function() {
      ...
      expect(Tournament3(pop, r)).to.equal(pop.genes[8])
    })
    it('should return first in tie', function() {
      const pop = { genes: _.range(10).map(i => wrapGene(i, 0)) }
      const r = new TestRandom(3, 6, 1)
      expect(Tournament3(pop, r)).to.equal(pop.genes[3])
    })
    it('should return larger negative fitness', function() {
      const pop = { genes: _.range(10).map(i => wrapGene(i, -i)) }
      const r = new TestRandom(2, 8, 5)
      expect(Tournament3(pop, r)).to.equal(pop.genes[2])
    })
  })
  describe('TournamentN (big -> 5)', function() {
    ...
    it('should return winner of tournament', function() {
      const pop = { genes: _.range(100).map(i => wrapGene(i, i)) }
      const r = new TestRandom(55, 3, 98, 27, 76)
      expect(t5(pop, r)).to.equal(pop.genes[98])
    })
    it('should return first in tie', function() {
      const pop = { genes: _.range(100).map(i => wrapGene(i, 0)) }
      const r = new TestRandom(22, 16, 91, 61, 43)
      expect(t5(pop, r)).to.equal(pop.genes[22])
    })
    it('should return larger negative fitness', function() {
      const pop = { genes: _.range(100).map(i => wrapGene(i, -i)) }
      const r = new TestRandom(32, 69, 41, 28, 91)
      expect(t5(pop, r)).to.equal(pop.genes[28])
    })
  })
})
